...
if __name__ == "__main__":
    assert len(argv) == 3
    ref = tonemapping(np.clip(cv.imread(argv[1], cv.IMREAD_UNCHANGED)[:, :, :3], 0, 10))
    test = tonemapping(np.clip(cv.imread(argv[2], cv.IMREAD_UNCHANGED)[:, :, :3], 0, 10))
    print(f"1-SSIM = {1 - ssim(ref, test, multichannel=True)}")
    print(f"PSNR = {psnr(ref, test)}")
