...
def plot_diff():
    ...
    folder = 'staircase'
    target_image = read_hdr2ldr(os.path.join(folder, "target-720-1024spp.exr"))
    initial_image = read_hdr2ldr(os.path.join(folder, "initial-720-512spp.exr"))
    optimizing_image = read_hdr2ldr(os.path.join(folder, "optimizing-720-512spp-iter3.exr"))
    final_image = read_hdr2ldr(os.path.join(folder, "final-720-1024spp.exr"))
    # diff_image = read_hdr2ldr(os.path.join(folder, "diff-720-1024spp.exr"))
    diff_image = final_image
    ...
    final_picture_ax.set_xlabel(f'Iteration #399\nOurs: {LuisaRender_time[0]/3600:.2f}h\nMitsuba 3: {Mitsuba3_time[0]/3600:.2f}h', fontsize=font_size)
    diff_picture_ax.set_title("Difference", fontsize=font_size)
    diff_picture_ax.set_xlabel(f"relMSE = {'TODO'}", fontsize=font_size)
