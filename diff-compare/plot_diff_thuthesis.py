...
def plot_diff():
    ...
    Mitsuba2_time = [1062.563411, 4.549333561, 6.076300551]

    for i in range(len(LuisaRender_time)):
        LuisaRender_time[i] *= 2
        Mitsuba2_time[i] *= 2

    folder = 'diff-compare'
    target_image = read_hdr2ldr(f"cbox-dragon-target.exr")
    initial_image = read_hdr2ldr(f"cbox-dragon-initial.exr")
    optimizing_image = read_hdr2ldr(f"cbox-dragon-iter3.exr")
    final_image = read_hdr2ldr(f"cbox-dragon-optimized.exr")
    images = [target_image, initial_image, final_image]

    gs = fig.add_gridspec(nrows=2, ncols=2, width_ratios=[1, 1], height_ratios=[1, 1], wspace=0.03, hspace=0.03)
    ...
    optimizing_picture_ax = fig.add_subplot(gs[1, 0])
    final_picture_ax = fig.add_subplot(gs[1, 1])
    # compare_ax = fig.add_subplot(gs[:, 2])
    ...
    optimizing_picture_ax.set_xlabel("Iteration #3\n(128spp per iteration)", fontsize=font_size)
...
Ours: {LuisaRender_time[0]:.0f}s / Mitsuba 2: {Mitsuba2_time[0]:.0f}s''', fontsize=font_size)
    ...
    plt.show()
    fig.savefig(f"cbox-diff-thuthesis.pdf", dpi=600, bbox_inches='tight')
    fig.savefig(f"cbox-diff-thuthesis.png", dpi=600, bbox_inches='tight')
